...
export const getOne: AppRouteHandler<GetOneRoute> = async (c: any) => {
  ...
  const [data] = await db.select({
    ...
    volume_number: volume.volume_number,
    no: volume.no,
    volume_id: sql`'volume' || COALESCE(${volume.volume_number}::text, '') || '-number' || COALESCE(${volume.no}::text, '') || '-' || to_char(${volume.published_date}, 'FMMonth') || COALESCE(to_char(${volume.published_date}, 'YYYY'), '')`.as('volume_id'),
    created_by: volume.created_by,
    created_by_name: users.name,
    created_at: volume.created_at,
    update_by: volume.updated_by,
