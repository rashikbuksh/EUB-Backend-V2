...
export const list: AppRouteHandler<ListRoute> = async (c: any) => {
  ...
  const { department_name, latest, is_pagination, access, is_global, field_name, field_value, type } = c.req.valid('query');
  ...
  if (type) {
    baseQuery.groupBy(news.uuid, news.type, department.name, department.short_name);
    baseQuery.having(eq(news.type, type));
  }
  else {
    baseQuery.groupBy(news.uuid, department.name, department.short_name);
    baseQuery.having(eq(news.type, 'general'));
  }
