...
export const list: AppRouteHandler<ListRoute> = async (c: any) => {
  ...
  const resultPromise = db.select({
    ...
    status: sql` CASE 
                    ...
                    WHEN ${capital.is_quotation} = true AND ${capital.is_cs} = true AND ${capital.cs_remarks} IS NOT NULL AND ${capital.is_monthly_meeting} = true AND ${capital.monthly_meeting_remarks} IS NOT NULL AND ${capital.is_work_order} = true AND ${capital.work_order_remarks} IS NOT NULL AND ${capital.done} = true THEN 'Paid'
                  END`,
    value: sql` CASE 
