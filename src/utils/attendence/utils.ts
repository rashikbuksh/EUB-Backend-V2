...
const CODE_TO_METHOD: { [key: string]: string } = {
  0: 'password',
  1: 'fingerprint',
  2: 'password',
  3: 'card',
...
export function verifyCodeToMethod(code: string | number): string {
  const c = String(code);

  return CODE_TO_METHOD[c] || 'unknown';
}

export function fmtYmdHms(date: any) {
  if (!date)
    return date;
  let d = date;
  if (typeof d === 'string') {
...
export function maxTimestampYmdHms(items: any[]) {
  let maxDate = null;
...
export function kvPairs(s: string) {
